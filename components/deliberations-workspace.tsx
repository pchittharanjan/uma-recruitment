    ...
    <PageContainer size="wide" className="flex min-h-0 flex-col gap-4">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-0 overflow-hidden rounded-lg border border-border bg-card">
        <div
          ...
          className="flex items-stretch gap-0 overflow-x-auto border-b border-border bg-muted/40"
        >
          {openTabIds.length === 0 ? (
            ...
            openTabIds.map((teamId) => {
              ...
              return (
                <div
                  key={teamId}
                  className={cn(
                    'group flex max-w-56 shrink-0 items-center gap-0.5 border-r border-border',
                    isActive
