  ...
  return (
    <div ref={rootRef} className="relative">
      ...
      <div
        role="menu"
        className={cn(
          'absolute right-0 top-full z-50 mt-2 w-56 rounded-lg border border-border bg-card p-2 transition-all',
          open ? 'visible opacity-100' : 'invisible pointer-events-none opacity-0',
        )}
      >
        <div className="px-2 py-2">
          <p className="truncate text-sm font-medium text-foreground">{user.name}</p>
          <p className="truncate text-xs normal-case text-muted-foreground">{user.email}</p>
