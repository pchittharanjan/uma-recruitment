    ...
    >
      ...
      >
        <div
          ...
          className="flex size-full flex-col bg-sidebar group-data-[variant=floating]:rounded-xl group-data-[variant=floating]:ring-1 group-data-[variant=floating]:ring-sidebar-border group-data-[variant=inset]:rounded-xl"
        >
          {children}
        </div>
      </div>
...
function SidebarRail({ className, ...props }: React.ComponentProps<"button">) {
  const { toggleSidebar, state, open, setOpen, width, setWidth, setIsResizing } =
    useSidebar()
  ...
  const handlePointerMove = (event: React.PointerEvent<HTMLButtonElement>) => {
    ...
    setWidth(drag.startWidth + delta, false)
  }
  ...
  const handlePointerUp = (event: React.PointerEvent<HTMLButtonElement>) => {
    ...
    else persistSidebarWidth(clampSidebarWidth(drag.startWidth + (event.clientX - drag.startX)))
  }
  ...
  return (
    <button
      ...
      title={
        state === "collapsed"
          ? "Expand Sidebar (⌘B)"
          : "Drag to resize · click to collapse (⌘B)"
      }
...
function SidebarInset({ className, ...props }: React.ComponentProps<"main">) {
  return (
    <main
      ...
      className={cn(
        "relative flex min-w-0 w-full flex-1 flex-col bg-background md:peer-data-[variant=inset]:m-2 md:peer-data-[variant=inset]:ml-0 md:peer-data-[variant=inset]:rounded-xl",
        className
      )}
      {...props}
    />
...
const sidebarMenuButtonVariants = cva(
  "peer/menu-button group/menu-button flex w-full items-center gap-2 overflow-hidden rounded-md p-2 text-left text-sm ring-sidebar-ring outline-hidden transition-[width,padding] group-has-data-[sidebar=menu-action]/menu-item:pr-8 group-data-[collapsible=icon]:!size-8 group-data-[collapsible=icon]:!p-2 group-data-[collapsible=icon]:[&>span:last-child]:hidden hover:bg-sidebar-accent-hover hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-open:hover:bg-sidebar-accent-hover data-open:hover:text-sidebar-accent-foreground data-active:bg-sidebar-accent data-active:font-medium data-active:text-sidebar-accent-foreground [&_svg]:size-4 [&_svg]:shrink-0 [&>span:last-child]:truncate",
  {
    variants: {
      variant: {
        default: "hover:bg-sidebar-accent-hover hover:text-sidebar-accent-foreground",
        outline:
          "bg-background ring-1 ring-sidebar-border hover:bg-sidebar-accent-hover hover:text-sidebar-accent-foreground hover:ring-sidebar-accent",
      },
      size: {
        default: "h-8 text-sm",
        sm: "h-7 text-xs group-data-[collapsible=icon]:!h-7",
        lg: "h-12 text-sm group-data-[collapsible=icon]:!size-8 group-data-[collapsible=icon]:!p-2 group-data-[collapsible=icon]:[&>div]:hidden group-data-[collapsible=icon]:[&>svg:last-child]:hidden",
      },
    },
    defaultVariants: {
      variant: "default",
